...
- ```{XU}```: **X** represent number of argument **U** meand URL encoding
- ```{X}```: **X** represent number of argument
...
#### PowerShell
Executing **PowerShell** commands is same as **CMD** commands and the function is exctly the same ```PowerShell(string commandWithArguments, string path, out string errors)```
#### Application
...
```
if your plugin contains **.config** file you need to provide the config file and set **HasConfig** needs to be true. If your plugin contains another **.dll**-s you will need to find a way to provied / specify them.
